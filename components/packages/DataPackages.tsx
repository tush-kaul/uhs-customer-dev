...
const DataPackages = ({ statusFilter }: { statusFilter: string }) => {
	...
	const data = React.useMemo(() => {
		...
		return rawData.filter((item: any) => {
			...
			const propertyMatch =
				propertyFilter === "all" ||
				item.property?.name === propertyFilter;

			// Apartment filter
			const apartmentMatch =
				apartmentFilter === "all" ||
				item.appartment_number === apartmentFilter;
	...
	const toggleRowExpansion = (itemId: string) => {
		...
		// Return focus to the same position after re-render
		setTimeout(() => {
			window.scrollTo({
					...
					<TooltipProvider delayDuration={100}>
						<Tooltip>
							...
							<TooltipContent
								side="top"
								sideOffset={5}
								className="bg-white p-2 shadow-lg rounded border border-gray-200 max-w-xs md:max-w-sm z-50"
							>
								<p className="text-xs text-gray-800">
									This booking will be cleared after 10
									minutes. Please continue with the booking
									process.
								</p>
							</TooltipContent>
						</Tooltip>
				...
				<TableBody>
					{dataLoading ? (
						...
						data
							?.sort((a: any, b: any) => {
								const numA = parseInt(
									a.booking_number.split("-")[1],
									10
								);
								const numB = parseInt(
									b.booking_number.split("-")[1],
									10
								);
								...
								return (
									<React.Fragment key={item.id}>
										<TableRow className="hover:bg-gray-50 flex-wrap">
											...
											<TableCell className="align-top">
												<p className="font-medium break-words min-w-[100px]">
														...
														>
															<Tooltip>
																...
																<TooltipContent
																	...
																	className="bg-white p-2 shadow-lg rounded border border-gray-200 max-w-xs md:max-w-sm"
																>
																	<p className="text-xs text-gray-800">
																		{item.has_renewed
																			? "This package has been renewed for a new term."
																			: "This is a renewal package created from a previous booking."}
																	</p>
																</TooltipContent>
															</Tooltip>
														</TooltipProvider>
	...
	const ModifiedPackageCard = ({
		...
		// Create ref for this card element
		const cardRef = React.useRef<HTMLDivElement>(null);

		// After expanding, ensure the card stays in view
		React.useEffect(() => {
			if (expandedRow === item.id && cardRef.current) {
				// Wait for rendering to complete
				setTimeout(() => {
					// Make sure the expanded section stays in view
					const cardPosition =
					...
					if (
						...
						// Only scroll if the bottom of the card is below the viewport
						window.scrollBy({
							top: Math.min(
								150,
								cardPosition.bottom - window.innerHeight + 50
							),
							behavior: "smooth",
						});
					}
				...
				<div className="p-4">
					<div className="flex items-center justify-between mb-2">
						...
						<Button
							...
							className="p-0 h-8 w-8 rounded-full focus:outline-none"
						>
							{expandedRow === item.id ? (
					...
					<div className="grid grid-cols-2 gap-2 mb-2 text-sm">
						<div>
							<span className="text-gray-500">Apt. No.:</span>
							<p className="font-medium break-words min-w-[100px]">
								{item?.appartment_number}
							</p>
						</div>
			...
			<div className="text-center py-16">
				...
				<p className="mt-1 text-sm text-gray-500">
					{propertyFilter !== "all" || apartmentFilter !== "all"
						...
						: `You don't have any ${statusFilter.replace(
								"_",
								" "
						  )} packages.`}
				</p>
